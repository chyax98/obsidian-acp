...
export function enhanceEnvForNodeScript(
	...
	if (cliPath.includes('/.nvm/')) {
		...
		if (binDirMatch) {
			...
			console.log(`[EnvUtils] 检测到 Unix nvm 路径，添加到 PATH: ${nvmBinDir}`);
			return env;
		}
	}
	...
	if (process.platform === 'win32' && /[/\\]nvm[/\\]v\d+\.\d+\.\d+[/\\]/i.test(cliPath)) {
		const binDirMatch = cliPath.match(/^(.+[/\\]nvm[/\\]v\d+\.\d+\.\d+)[/\\]/i);
		if (binDirMatch) {
			const nvmBinDir = binDirMatch[1];
			const pathSeparator = ';';
			env.PATH = `${nvmBinDir}${pathSeparator}${env.PATH || ''}`;
			...
			// eslint-disable-next-line no-console
			console.log(`[EnvUtils] 检测到 Windows nvm 路径，添加到 PATH: ${nvmBinDir}`);
			return env;
		}
	}
