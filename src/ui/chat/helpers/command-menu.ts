...
export class CommandMenuHelper {
	...
	public show(filter = ''): void {
		...
		filteredCommands.forEach((cmd, index) => {
			...
			item.addEventListener('mouseenter', () => {
				this.menuEl?.querySelectorAll('.acp-command-menu-item').forEach((el, i) => {
	...
	public navigate(direction: 'up' | 'down'): void {
		...
		// 移除当前选中
		items[this.selectedIndex]?.classList.remove('acp-command-menu-item-selected');
	...
	public async select(command?: AvailableCommand): Promise<void> {
		...
		if (!command && this.menuEl) {
			const items = Array.from(this.menuEl.querySelectorAll('.acp-command-menu-item'));
			const selectedItem = items[this.selectedIndex];
			if (selectedItem) {
				const commandName = selectedItem.querySelector('.acp-command-menu-name')?.textContent?.slice(1);
