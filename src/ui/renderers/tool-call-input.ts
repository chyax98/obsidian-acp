...
export class ToolCallInputRenderer {
	...
	private static renderFileOperationInput(
		...
		// 文件路径
		const filePath = getStringParam(rawInput, ['path', 'file_path', 'filename']);
		if (filePath) {
		...
		// 其他参数
		const otherParams = getOtherParams(rawInput, ['path', 'file_path', 'filename', 'old_string', 'new_string', 'content']);
	 ...
	 * 渲染文件路径
	 */
	private static renderFilePath(container: HTMLElement, filePath: string, app?: App): void {
	 ...
	 * 渲染编辑内容
	 */
	private static renderEditContent(container: HTMLElement, rawInput: Record<string, unknown>): void {
	 ...
	 * 渲染写入内容
	 */
	private static renderWriteContent(container: HTMLElement, content: string): void {
	 ...
	 * 渲染命令输入
	 */
	private static renderCommandInput(container: HTMLElement, rawInput: Record<string, unknown>): void {
	...
	private static renderSearchInput(container: HTMLElement, rawInput: Record<string, unknown>): void {
		...
		const otherParams = getOtherParams(rawInput, ['pattern', 'query', 'search', 'path', 'directory', 'glob', 'include']);
		if (Object.keys(otherParams).length > 0) {
			this.renderOtherParams(container, otherParams);
		}
	}
