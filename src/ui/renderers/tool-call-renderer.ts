...
export class ToolCallRenderer {
	 ...
	 * 渲染工具调用卡片
	 */
	public static render(container: HTMLElement, toolCall: ToolCall, app?: App): HTMLElement {
		// 查找是否已存在
	 ...
	 * 创建新的工具调用卡片
	 */
	private static createCard(container: HTMLElement, toolCall: ToolCall, app?: App): HTMLElement {
		...
		// 显示关键参数
		const paramsPreview = ToolCallDisplayHelper.formatParamsPreview(toolCall.rawInput, toolCall.kind);
		if (paramsPreview) {
		...
		setIcon(chevronEl, shouldExpandByDefault ? 'chevron-down' : 'chevron-right');
	 ...
	 * 更新工具调用卡片
	 */
	private static updateCard(toolCallEl: HTMLElement, toolCall: ToolCall, app?: App): void {
			...
			if (toolCall.content && toolCall.content.length > 0) {
				...
				if (contentEl) {
					...
					if (currentContentCount !== toolCall.content.length) {
						...
						ToolCallContentRenderer.render(contentEl as HTMLElement, toolCall, app);
					}
				}
			}
		});
	 ...
	 * 设置状态图标
	 */
	private static setStatusIcon(iconEl: HTMLElement, status: ToolCall['status']): void {
		iconEl.empty();
